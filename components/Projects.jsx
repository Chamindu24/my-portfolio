...
const projects = [
  ...
  },
  {
    ...
    description: `Created a smart note-taking app integrated with Gemini API to summarize and enhance text inputs using natural language processing`,
    image: "/calc.jpg",
    ...
    <div className="relative z-10 container mx-auto text-center px-6 lg:px-20">
      ...
      <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 items-center justify-items-center">
        {projects.map((project, index) => (
          <div
            key={index}
            className="group relative rounded-xl bg-gray-800 p-6 shadow-xl transition-all duration-300 transform hover:scale-105 hover:shadow-2xl border-2 border-transparent hover:border-indigo-200"
          >
            {/* Project Image */}
            <Image
              src={project.image}
            ...
            {/* Project Title */}
            <h3 className="text-xl font-semibold text-white mt-5 font-mono">
              {project.title}
            </h3>
            ...
            {/* GitHub Link with Icon - FIXED VERSION */}
            <a
              href={project.link}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-6 inline-flex items-center text-blue-500 font-medium group-hover:text-indigo-400 hover:underline transition-all duration-300 cursor-pointer"
              onClick={(e) => {
                // Debug: log the link being clicked
                console.log('Clicking link:', project.link);
            ...
            <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-indigo-400 to-transparent opacity-5 group-hover:opacity-20 transition-all duration-500 rounded-lg blur-2xl pointer-events-none"></div>
          </div>
        ))}
      </div>
    </div>
